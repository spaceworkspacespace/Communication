...
class ProfileController extends UserBaseController
{
    ...
    public function editPost()
    {
        if ($this->request->isPost()) {
            ...
            if ($editData->editData($data)) {
                $this->success(lang('EDIT_SUCCESS'), "/im/profile/center");
            } else {
    ...
    public function avatarUpload()
    {
        ...
        $result = $file->validate([
            'ext'  => 'jpg,jpeg,png',
            'size' => 1024 * 1024
        ])->rule("md5")->move($folder);

        if ($result) {
            
            $avatar = implode(["/upload/",$result->getSaveName() ]);
    ...
    public function avatarUpdate()
    {
        ...
        if (!empty($avatar)) {
            ...
            $avatarPath =  implode(DIRECTORY_SEPARATOR,[
                ROOT_PATH,
                "public",
                "upload",
                str_replace("/upload/", "", $avatar)
            ]);
            
            $saveTempPath = implode(DIRECTORY_SEPARATOR, [
                ROOT_PATH,
                "public",
                "upload",
                "tmp",
                "avatar-".cmf_get_current_user_id()."-".time().strrchr($avatar, "."),
            ]);
            
            //创建文件
            fopen($saveTempPath, 'w');
            
//             var_dump($avatarPath);
//             var_dump($saveTempPath);
            
            // 复制文件
            copy($avatarPath, $saveTempPath);
            
            $avatarImg = Image::open($saveTempPath);
            $avatarImg->crop($w, $h, $x, $y)->save($saveTempPath);
            ...
            $newSavePath = implode([
                ...
                "upload",
                DIRECTORY_SEPARATOR,
                md5_file($saveTempPath).strrchr($avatar, "."),
            ]);
            ...
            //移动文件
            rename($saveTempPath, $newSavePath);
            
            //生成网络地址
            $avatar = implode([ "/upload/",basename($newSavePath) ]);
